---
name: rate-it
description: Rate a codebase, module, or diff across six axes — code quality, performance, design (frontend), system design, technical debt, and overall — each scored /100 with evidence and fair justification. Use when the user says "/rate-it", "rate this", "score this code", "how good is this", or wants a quality report before/after work. Read-only - never modifies files.
---

# Rate It

Score the target across six axes, each with evidence and a fair justification. This is a measurement tool, not a fix tool — never modify files during a rating.
...
1. Establish scope: whole repo, one module, or a diff. For a whole repo, read the docs/entry points first (`CLAUDE.md`, `README`, main modules), then sample the riskiest paths (money, auth, concurrency) — do not read every file.
2. For large scope, spawn one fresh-context subagent per heavy axis (quality / performance+system-design / debt) and aggregate; for small scope, rate inline. A fresh rater must not be the author of the code being rated.
3. Score each axis with 2-4 evidence bullets (the deductions) + 1-2 "already right" bullets.
4. Rank the top 3 highest-impact fixes across all axes — impact per effort, not severity theater.
...
After fixes, re-rate with a fresh context (or fresh subagent) — never let the session that wrote the fixes grade its own work. Compare axis-by-axis and state what moved and why.
